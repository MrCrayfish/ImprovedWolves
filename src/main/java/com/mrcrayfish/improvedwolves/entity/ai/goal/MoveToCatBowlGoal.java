 ...
 * Author: MrCrayfish
 */
public class MoveToCatBowlGoal extends Goal
{
    private int eatingTicks;
    private BlockPos bowlPos;
    private CatBowlTileEntity bowlTileEntity;
    private TameableEntity entity;

    public MoveToCatBowlGoal(TameableEntity entity)
    {
        this.entity = entity;
        ...
        else if(!this.bowlTileEntity.isRemoved() && this.bowlTileEntity.hasFood())
        {
            ...
            else
            {
                ...
                if(world instanceof ServerWorld)
                {
                    ...
                    ItemStack food = new ItemStack(Items.COOKED_COD);
    ...
    private void findNearestCatBowl()
    {
        BlockPos center = new BlockPos(this.entity);
        List<BlockPos> catBowls = new ArrayList<>();
        BlockPos.getAllInBox(center.add(-10, -10, -10), center.add(10, 10, 10)).forEach(pos ->
        {
            if(this.entity.getEntityWorld().getBlockState(pos).getBlock() instanceof CatBowlBlock)
            {
                catBowls.add(pos.toImmutable());
            }
        });
        BlockPos bowlPos = catBowls.stream().min(Comparator.comparing(pos -> pos.manhattanDistance(center))).orElse(null);
        if(bowlPos != null)
        {
            TileEntity tileEntity = this.entity.getEntityWorld().getTileEntity(bowlPos);
            if(tileEntity instanceof CatBowlTileEntity)
            {
                CatBowlTileEntity catBowlTileEntity = (CatBowlTileEntity) tileEntity;
                if(catBowlTileEntity.hasCustomName())
                {
                    if(!catBowlTileEntity.getName().getUnformattedComponentText().equals(this.entity.getName().getUnformattedComponentText()))
                    {
                        return;
                    }
                }
                this.bowlPos = bowlPos;
                this.bowlTileEntity = (CatBowlTileEntity) tileEntity;
            }
        }
    }
}
